...
# Create a GuardDuty publishing destination
resource "aws_guardduty_publishing_destination" "s3_destination" {
  count = var.enabled_export_to_s3 ? 1 : 0

  detector_id     = data.aws_guardduty_detector.current.id
  destination_arn = var.guard_duty_s3_bucket_arn
  kms_key_arn     = var.guard_duty_kms_arn
